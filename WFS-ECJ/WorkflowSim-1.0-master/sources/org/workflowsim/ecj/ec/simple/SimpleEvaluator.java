...
public class SimpleEvaluator extends Evaluator
{
    ...
    /**
     * A private helper function for evaluatePopulation which evaluates a chunk of
     * individuals in a subpopulation for a given thread. Although this method is
     * declared protected, you should not call it.
     */
    ...
    protected void evalPopChunk(EvolutionState state, int[] numinds, int[] from, int threadnum, SimpleProblemForm p) {
        ...
        for (int pop = 0; pop < len; pop++) {
            ...
            ArrayList<Double> totaltime_list = new ArrayList<Double>();
            GPIndividual gpInd = null;

            for (int x = fp; x < upperbound; x++) {
                gpInd = (GPIndividual) inds.get(x);
                totaltime_list.add(gpInd.getTotalTime());
            }

            Collections.sort(totaltime_list);
            double min_time = totaltime_list.get(0);
            Collections.reverse(totaltime_list);
            double max_time = totaltime_list.get(0);

            for (int x = fp; x < upperbound; x++) {
                gpInd = (GPIndividual) inds.get(x);
                double fitness = gpInd.getTotalTime()*(-1);
                SimpleFitness f = ((SimpleFitness) inds.get(x).fitness);
