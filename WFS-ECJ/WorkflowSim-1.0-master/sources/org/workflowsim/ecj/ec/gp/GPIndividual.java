...
/**
 * GPIndividual is an Individual used for GP evolution runs. GPIndividuals
 * contain, at the very least, a nonempty array of GPTrees. You can use
 * GPIndividual directly, or subclass it to extend it as you see fit.
 *
 * <P>
 * GPIndividuals have two clone methods: clone() and lightClone(). clone() is a
 * deep clone method as usual. lightClone() is a light clone which does not copy
 * the trees.
 *
 * <p>
 ...
 * approach transmits or receives an indivdual in text encoded such that the
 * individual is largely readable by humans but can be read back in 100% by ECJ
 * as well. Because GPIndividuals are often very large, <b>GPIndividual has
 * overridden these methods -- they work differently than in Individual (the
 * superclass).</b> In specific: <b>readIndividual</b> by default reads in the
 * fitness and the evaluation flag, then calls <b>parseGenotype</b> to read in
 * the trees (via GPTree.readTree(...)). However <b>printIndividual</b> by
 * default prints the fitness and evaluation flag, and prints all the trees by
 * calling GPTree.printTree(...). It does not call <b>genotypeToString</b> at
 * all. This is because it's very wasteful to build up a large string holding
 * the printed form of the GPIndividual just to pump it out a stream once.
 *
 ...
 * only. Because GPIndividuals are often very large, <b>GPIndividual has
 * overridden this methods -- it works differently than in Individual (the
 * superclass).</b> In specific: <b>printIndividual</b> by default prints the
 * fitness and evaluation flag, and prints all the trees by calling
 * GPTree.printTreeForHumans(...). It does not call
 * <b>genotypeToStringForHumans</b> at all. This is because it's very wasteful
 * to build up a large string holding the printed form of the GPIndividual just
 * to pump it out a stream once.
 *
 * <p>
 ...
 * read/write the GPTree array (by having each GPTree read/write itself). If you
 * add instance variables to GPIndividual, you'll need to read/write those
 * variables as well.
 *
 *
 * <p>
 ...
 * <table>
 * <tr>
 * <td valign=top><i>base</i>.<tt>numtrees</tt><br>
 * <font size=-1>int &gt;= 1</font></td>
 * <td valign=top>(number of trees in the GPIndividual)</td>
 * </tr>
 *
 * <tr>
 * <td valign=top><i>base</i>.<tt>tree.</tt><i>n</i><br>
 * <font size=-1>classname, inherits or = ec.gp.GPTree</font></td>
 * <td valign=top>(class of tree <i>n</i> in the individual)</td>
 * </tr>
 * </table>
 *
 * <p>
 ...
 * <table>
 * <tr>
 * <td valign=top><i>base</i>.<tt>tree.</tt><i>n</i></td>
 * <td>tree <i>n</i> in the individual</td>
 * </tr>
 * </table>
 *
 *
...
public class GPIndividual extends Individual {
    ...
    public Map<String, Double> workflowStats;

    public Map<String, Double> workflowMkspns;
    ...
    public int hashCode() {
        ...
        for (int x = 0; x < trees.length; x++)
            hash =
                    // Rotate hash and XOR
                    (hash << 1 | hash >>> 31) ^ trees[x].treeHashCode();
        return hash;
    }

    /**
     * Sets up a prototypical GPIndividual with those features which it shares with
     * other GPIndividuals in its species, and nothing more.
     */
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        //setup the workflow map
        this.workflowStats = new HashMap<>();
        ...
        if (t <= 0)
            state.output.fatal("A GPIndividual must have at least one tree.", base.push(P_NUMTREES),
                    def.push(P_NUMTREES));
        ...
        for (int x = 0; x < t; x++) {
            ...
            trees[x] = (GPTree) (state.parameters.getInstanceForParameterEq(p, def.push(P_TREE).push("" + x),
                    GPTree.class));
            trees[x].owner = this;
    ...
    /**
     * Verification of validity of the GPIndividual -- strictly for debugging
     * purposes only
     */
    ...
    /**
     * Prints just the trees of the GPIndividual. Broken out like this to be used by
     * GEIndividual to avoid re-printing the fitness and evaluated premables.
     */
    ...
    public void readGenotype(final EvolutionState state, final DataInput dataInput) throws IOException {
        ...
        if (trees == null || treelength != trees.length) // wrong size!
            state.output.fatal(
                    "Number of trees differ in GPIndividual when reading from readGenotype(EvolutionState, DataInput).");
        for (int x = 0; x < trees.length; x++)
            trees[x].readTree(state, dataInput);
    }

    public void parseGenotype(final EvolutionState state, final LineNumberReader reader) throws IOException {
        // Read my trees
        for (int x = 0; x < trees.length; x++) {
    ...
    /**
     * Deep-clones the GPIndividual. Note that you should not deep-clone the
     * prototypical GPIndividual stored in GPSpecies: they contain blank GPTrees
     * with null roots, and this method, which calls GPTree.clone(), will produce a
     * NullPointerException as a result. Instead, you probably want to use
     * GPSpecies.newIndividual(...) if you're thinking of playing with the
     * prototypical GPIndividual.
     */
    ...
    /**
     * Returns the "size" of the individual, namely, the number of nodes in all of
     * its subtrees.
     */
    public long size() {
        long size = 0;
    ...
    public Map<String, Double> getWorkflowStats(){
        return this.workflowStats;
    }

    public void setWorkFlowStats(Map<String,Double> workflowStats){
